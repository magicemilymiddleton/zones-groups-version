...
licensesData.forEach(license => {
    // Use the actual property names from your data
    const courseId = license.product_id;  // Change from license.course_id
    const courseTitle = license.course_title;
    const passKey = `${license.ID}_${license.product_id}`;  // Use license.ID and product_id
    ...
    if (!passInventory[passKey]) {
        passInventory[passKey] = {
            pass_id: license.ID,  // Use license.ID instead of license.pass_id
            course_id: courseId,  // This will now be product_id
            course_title: courseTitle,
            pass_title: courseTitle,  // Or whatever property has the title
            start_date: license.start_date || license.start_date_formatted,
            licenses: [],
            available: 0
        };
    }
    ...
    function getAvailableCountForCourse(courseId) {
        ...
        // DEBUG
    console.log('=== getAvailableCountForCourse DEBUG ===');
    console.log('Looking for courseId:', courseId);
    console.log('passInventory keys:', Object.keys(passInventory));
    console.log('passInventory:', passInventory);
