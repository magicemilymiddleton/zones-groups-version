...
<?php
global $wpdb;
$debug_all = $wpdb->get_var($wpdb->prepare(
    "SELECT COUNT(*) FROM {$wpdb->posts} p
     INNER JOIN {$wpdb->postmeta} pm ON p.ID = pm.post_id
     WHERE p.post_type = 'llms_group_order' 
     AND pm.meta_key = 'group_id' AND pm.meta_value = %d",
    $group_id
));

$debug_assigned = $wpdb->get_var($wpdb->prepare(
    "SELECT COUNT(*) FROM {$wpdb->posts} p
     INNER JOIN {$wpdb->postmeta} pm_group ON p.ID = pm_group.post_id
     INNER JOIN {$wpdb->postmeta} pm_student ON p.ID = pm_student.post_id
     WHERE p.post_type = 'llms_group_order'
     AND pm_group.meta_key = 'group_id' AND pm_group.meta_value = %d
     AND pm_student.meta_key = 'student_id' 
     AND pm_student.meta_value != '' 
     AND pm_student.meta_value IS NOT NULL",
    $group_id
));

echo "<!-- Debug: Total orders: $debug_all, Assigned: $debug_assigned, Should be available: " . ($debug_all - $debug_assigned) . " -->";
echo "<!-- Debug: get_available_licenses returns: " . count($available_licenses) . " -->";
?>
...
jQuery(document).ready(function($) {
    console.log('Initializing enhanced multi-pass bulk assign system...');
    
    // ========== KEEP YOUR EXISTING CORE FUNCTIONS ==========
    // Keep updateBulkActionsBar, showSuccessMessage, etc.
    
    // ========== ADD THIS OVERRIDE SECTION AT THE END ==========
    
    // Wait for all scripts to load, then override the bulk assign handler
    setTimeout(function() {
        console.log('Activating enhanced multi-pass bulk assignment...');
        
        // Remove any existing bulk assign button handlers
        $(document).off('click', '#llmsgaa-bulk-assign-btn');
        $('#llmsgaa-bulk-assign-btn').off('click');
        
        // Attach the new enhanced handler
        $(document).on('click', '#llmsgaa-bulk-assign-btn', function(e) {
            ...
            console.log('Bulk assign clicked. Selected emails:', selectedEmails);
            
            if (selectedEmails.length === 0) {
                alert('Please select at least one member to assign passes to.');
                return;
            }
            
            // Check if enhanced function exists
            if (typeof showEnhancedBulkAssignModal === 'function') {
                console.log('Launching enhanced multi-pass modal...');
                showEnhancedBulkAssignModal(selectedEmails);
            } else {
                console.error('Enhanced bulk assign function not found!');
                console.log('Available functions:', Object.keys(window).filter(k => k.includes('bulk') || k.includes('Bulk')));
                alert('Enhanced bulk assign system is not loaded. Please ensure llmsgaa-bulk-assign.js is included and refresh the page.');
            }
        });
        
        // Also remove the old modal functions to prevent confusion
        if (typeof window.showBulkAssignModal !== 'undefined') {
            console.log('Removing old bulk assign modal function...');
            delete window.showBulkAssignModal;
        }
        if (typeof window.processBulkAssignment !== 'undefined') {
            delete window.processBulkAssignment;
        }
        if (typeof window.closeBulkModal !== 'undefined') {
            delete window.closeBulkModal;
        }
        
        console.log('✅ Enhanced multi-pass bulk assignment ready!');
        
    }, 500); // Small delay to ensure all scripts are loaded
});

// 5. ENSURE llmsgaa-bulk-assign.js IS LOADED

// Debug: Check if the enhanced JS file is loaded
jQuery(document).ready(function($) {
    // Add a visual indicator that the new system is active
    setTimeout(function() {
        if (typeof showEnhancedBulkAssignModal === 'function') {
            $('#llmsgaa-bulk-assign-btn').attr('title', 'Enhanced Multi-Pass Assignment (New!)');
            console.log('✅ Enhanced bulk assign JS is loaded and ready');
        } else {
            console.error('❌ Enhanced bulk assign JS not loaded - check if llmsgaa-bulk-assign.js is being enqueued');
            
            // Check if the file is in the page
            const scripts = Array.from(document.scripts).map(s => s.src);
            const hasBulkAssignJS = scripts.some(s => s.includes('llmsgaa-bulk-assign.js'));
            
            if (!hasBulkAssignJS) {
                console.error('llmsgaa-bulk-assign.js is NOT included in the page!');
                console.log('You need to enqueue this file. Add to your PHP:');
                console.log("wp_enqueue_script('llmsgaa-bulk-assign', plugin_dir_url(__FILE__) . 'public/js/llmsgaa-bulk-assign.js', ['jquery'], '2.0.0', true);");
            }
        }
    }, 1000);
});

// 6. OPTIONAL BUT RECOMMENDED: Add a temporary test button to verify the new modal works
jQuery(document).ready(function($) {
    // Remove this after testing
    if (window.location.hash === '#test-enhanced') {
        $('body').append(`
            <div style="position:fixed;bottom:20px;right:20px;z-index:99999;background:#4CAF50;padding:10px;border-radius:5px;">
                <button id="test-enhanced-modal" style="background:white;padding:5px 10px;border:none;border-radius:3px;cursor:pointer;">
                    Test Enhanced Modal
                </button>
            </div>
        `);
        
        $('#test-enhanced-modal').on('click', function() {
            if (typeof showEnhancedBulkAssignModal === 'function') {
                showEnhancedBulkAssignModal(['test1@example.com', 'test2@example.com']);
            } else {
                alert('Enhanced modal function not found!');
            }
        });
    }
});
