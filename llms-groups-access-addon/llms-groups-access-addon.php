 ...
 * 11/5/25 modified to reduce log events
 */
...
require_once LLMSGAA_DIR . 'includes/Feature/FluentCRMIntegration.php';
...
add_action( 'init', function() {
    ...
    add_filter( 'query', function( $query ) {
        //modified filter to reduce events
		if ( strpos( $query, 'lifterlms_user_postmeta' ) !== false && strpos( $query, '_group_role' ) !== false )
 {
    ...
    add_action( 'llmsgaa_debug_check_user_groups', function( $user_id ) {
        ...
        if ( $roles ) {
            ...
            // commented out because it's causing a loop? 11/5/25 DS
            // Force webhook send
