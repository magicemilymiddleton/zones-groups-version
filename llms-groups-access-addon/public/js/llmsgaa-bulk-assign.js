...
// Supports assigning multiple passes to the same member
...
jQuery(document).ready(function($) {
    ...
    window.showEnhancedBulkAssignModal = function(selectedEmails) {
        ...
        $.ajax({
            ...
            error: function() {
                alert('Error loading available licenses.');
            }
        });
    };
    ...
    window.confirmMultiAssignments = function() {
        ...
        // Prepare assignment data with multi-pass support
        ...
        if (assignmentData.length === 0) {
            alert('No assignments to process.');
            return;
        }
        
        // Show confirmation
        const membersWithAssignments = Object.values(memberAssignments).filter(a => a.passes.length > 0).length;
        if (!confirm(`Confirm assignment of ${assignmentData.length} passes to ${membersWithAssignments} members?`)) {
            return;
        }
        
        // Disable button during processing
        $('#confirm-multi-assignments-btn').prop('disabled', true).text('Processing...');
        

// Add the debug console.log right BEFORE the $.ajax call
console.log('Debug - Sending AJAX request with:', {
    action: 'llmsgaa_bulk_assign_multi_pass',
    assignments_count: assignmentData.length,
    group_id: group_id,
    nonce: nonce,
    ajaxurl: ajaxurl,
    sample_assignment: assignmentData[0]
});
...
$.ajax({
    ...
    success: function(response) {
        if (response.success) {
            closeEnhancedModal();
            alert(`Successfully assigned ${response.data.assigned_count} passes!`);
            location.reload();
        } else {
            alert('Error: ' + (response.data || 'Unknown error occurred'));
            $('#confirm-multi-assignments-btn').prop('disabled', false).text('✅ Confirm Assignments');
        }
    },
    error: function(xhr, status, error) {  // REPLACE the simple error function with this detailed one
        ...
        // Try to parse WordPress error
        let errorMsg = 'Server error occurred. Please try again.';
        try {
            if (xhr.responseText) {
                if (xhr.responseText.includes('0')) {
                    errorMsg = 'Action not found or nonce verification failed.';
                } else if (xhr.responseText.includes('Fatal error')) {
                    errorMsg = 'PHP error occurred. Check server logs.';
                }
            }
        } catch(e) {}
...
// Add this to your llmsgaa-bulk-assign.js (or llsmgaa-bulk-assign.js)
window.showEnhancedBulkAssignModal = function(selectedEmails) {
    const group_id = window.llmsgaa_group_id || window.groupId;
    const nonce = window.ajaxNonce;
    
    // Get available licenses grouped by course
    $.ajax({
        url: ajaxurl,
        type: 'POST',
        data: {
            action: 'llmsgaa_get_grouped_licenses',
            group_id: group_id,
            nonce: nonce
        },
        success: function(response) {
            if (response.success) {
                createAdvancedAssignmentMatrix(selectedEmails, response.data);
            }
        }
    });
};

function createAdvancedAssignmentMatrix(emails, licensesGroupedByCourse) {
    let html = `
    <div id="enhanced-bulk-modal" style="position: fixed; inset: 0; background: rgba(0,0,0,0.5); z-index: 99999; display: flex; align-items: center; justify-content: center;">
        <div style="background: white; border-radius: 8px; padding: 30px; max-width: 90%; max-height: 90%; overflow: auto;">
            <h2>Advanced License Assignment</h2>
            
            <div style="margin: 20px 0;">
                <h3>Available Licenses:</h3>
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; margin-bottom: 20px;">
                    ${Object.entries(licensesGroupedByCourse).map(([courseId, data]) => `
                        <div style="padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
                            <strong>${data.title}</strong><br>
                            <small>${data.available} available</small>
                        </div>
                    `).join('')}
                </div>
            </div>
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <thead>
                    <tr>
                        ...
                        ${Object.entries(licensesGroupedByCourse).map(([courseId, data]) => 
                            `<th style="border: 1px solid #ddd; padding: 8px;">${data.title}</th>`
                        ).join('')}
                    </tr>
                </thead>
                <tbody>
                    ${emails.map(email => `
                        <tr>
                            ...
                            ${Object.keys(licensesGroupedByCourse).map(courseId => `
            ...
            <div style="margin-top: 20px;">
                <h4>Quick Actions:</h4>
                <button onclick="selectAllForCourse()" class="button">Select All for Course</button>
                <button onclick="applyPattern()" class="button">Apply Pattern</button>
            ...
            <div style="margin-top: 20px; text-align: right;">
                <button onclick="closeEnhancedModal()" class="button">Cancel</button>
                <button onclick="confirmBulkAssignments()" class="button button-primary">Confirm Assignments</button>
            </div>
        </div>
    </div>`;
    ...
    $('.course-assignment').on('change', updateAssignmentSummary);
...
function updateAssignmentSummary() {
    ...
    let summary = {};
    $('.course-assignment:checked').each(function() {
        const course = $(this).data('course');
        summary[course] = (summary[course] || 0) + 1;
    });
    
    // Display summary
    let summaryHtml = Object.entries(summary).map(([course, count]) => 
        `<div>${course}: ${count} assignments</div>`
    ).join('');
    
    $('#assignment-summary').html(summaryHtml || '<div>No assignments selected</div>');
}