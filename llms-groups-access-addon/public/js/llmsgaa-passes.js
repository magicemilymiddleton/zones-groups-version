...
//passes.php also updated to handle this option

// Config: prefix that triggers automatic activation when any SKU starts with this value.
const LLMSGAA_AUTO_ACTIVATE_PREFIX = "train-stream";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".llmsgaa-redeem-btn").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      // Try to auto-activate when this order contains a configured SKU
      let items = [];
      try {
      ...
      const skus = (items || [])
        .map((i) => (i && i.sku ? String(i.sku) : ""))
        .filter(Boolean);
      const autoSkus = window.llmsgaaAutoActivateSkus || [];
      // Auto-activate when any SKU starts with the configured prefix OR matches configured SKUs
      const shouldAutoActivate =
        skus.some((s) => String(s).startsWith(LLMSGAA_AUTO_ACTIVATE_PREFIX)) ||
        skus.some((s) => autoSkus.indexOf(s) !== -1);

      if (shouldAutoActivate) {
        const form = document.getElementById("llmsgaa-redeem-form");
        if (form) {
          const passIdInput = form.querySelector('[name="pass_id"]');
          if (passIdInput) passIdInput.value = passId;

          const dateInput =
            form.querySelector('[name="start_date"]') ||
            form.querySelector("#llmsgaa-start-date");
          const today = new Date().toISOString().split("T")[0];
          if (dateInput) {
          ...
          // Submit form to activate immediately
          form.submit();
          return;
        }
      }

      // Fallback: open the redeem modal as before
...
function updateBulkActionsBar() {
  ...
  if (selectedCount > 0) {
    ...
    $("#llmsgaa-bulk-assign-btn")
      .removeClass("btn-primary")
      .addClass("btn-success");
    $("#llmsgaa-bulk-assign-btn").html(
      `<span class="btn-icon">📋</span> Bulk Assign (${selectedCount} selected)`
    );
  } else {
...
$(document).on("click", "#llmsgaa-bulk-assign-btn", function (e) {
  ...
  const selectedEmails = $(".member-checkbox:checked")
    .map(function () {
      return this.value;
    })
    .get();
...
$(document).ready(function () {
  ...
  if ($("#llmsgaa-bulk-assign-btn").length === 0) {
    console.error(
      "Bulk Assign button not found! Make sure the HTML includes the button."
    );
  } else {
