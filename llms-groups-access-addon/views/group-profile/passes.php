...
2.4n
added column sort
Remove User button visible for WP admins
*/

/*
 * Add passes selection option when bulk assign by Abhijit
 * showBulkAssignModal funcation replace with my new function (showBulkAssignModal) and commented old funcation
 */

//DS mod. 1/6/26: added lmsgaa_auto_activate_skus to skip date selection dialog for Livestream events (when SKU contains 'train-stream')
//lmsgaa_passes.js also updated to handle this option 
...
// Sort passes by most recent purchase date (newest first)
usort($passes, function($a, $b) {
    return strtotime(get_the_date('Y-m-d', $b->ID)) - strtotime(get_the_date('Y-m-d', $a->ID));
});
 ...
 <?php echo esc_html( $order_number ); ?>
  ...
  <?php endif; ?>
          ...
          <td style="vertical-align: middle;">
                        <?php if ( ! $is_redeemed ): ?>
                            <?php
                                // Ensure items are JSON-serializable for the data attribute
                                $pass_items_attr = esc_attr( wp_json_encode( $items ) );
                            ?>
                            <button
                                type="button"
                                class="llmsgaa-redeem-btn"
                                data-pass-id="<?php echo esc_attr( $p->ID ); ?>"
                                data-items="<?php echo $pass_items_attr; ?>"
    ...
    <script>
        // Expose configured SKUs (option: llmsgaa_auto_activate_skus) to front-end JS.
        window.llmsgaaAutoActivateSkus = <?php echo wp_json_encode( (array) get_option( 'llmsgaa_auto_activate_skus', array() ) ); ?> || [];
    </script>
...
<div class="llmsgaa-box">
    ...
    <!-- Bulk Assign Button -->
      <button id="llmsgaa-bulk-assign-btn" class="btn btn-primary" style="display: none; margin-left: 2rem;">
        <span class="btn-icon">📋</span> Bulk Assign
		      ...
		      <!-- CSV Export Button -->
      <button id="llmsgaa-csv-export-btn" class="btn btn-secondary">
        <span class="btn-icon">📥</span> Download Users CSV
      </button>
    </div>
  ...
  <?php endif; ?>
    ...
    <!-- Members Table -->
<div class="overflow-x-auto">
  <table id="llmsgaa-users-table" class="llmsgaa-table w-full text-sm">

      <thead>
  <tr>
    <th><input type="checkbox" id="select-all-members" aria-label="Select all members" /></th>
    <th>Name / Email</th>
    <th>Role</th>
    <th>Last Activity</th>
    <th>Access Passes</th>
...
?>
<tr class="member-row" data-email="<?php echo esc_attr( $member['email'] ); ?>" data-user-id="<?php echo esc_attr( $member['user_id'] ?: '' ); ?>" data-last-login-raw="<?php echo esc_attr( $member['last_login_raw'] ?? 0 ); ?>">
  <td>
    <input type="checkbox" class="member-checkbox" value="<?php echo esc_attr( $member['email'] ); ?>" aria-label="Select member" />
  </td>
  <td>
    <div>
...
?>
<select aria-label="Select user role" class="role-select border rounded px-2 py-1" data-email="<?php echo esc_attr( $member['email'] ); ?>" data-user-id="<?php echo esc_attr( $member['user_id'] ?: '' ); ?>">
...
<!--  supress Admin badge
 ...
 <!-- Cancel Invite and Remove buttons moved to last login class=btn-xs--> 
  ...
  <?php if ( $member['status'] === 'pending' ): ?>
	<button class="btn btn-xs btn-danger cancel-invite-btn"
	  title="Cancel invitation email and remove Access Passes"
	  data-invite-id="<?php echo esc_attr( $member['invite_id'] ); ?>">
	  <span class="btn-icon">✖</span> Cancel Invite
	</button>
  <?php else: ?>
	<?php if ( current_user_can( 'manage_options' ) ) : ?>
	  <!-- Only WP admins see this -->
	  <button class="btn btn-xs btn-danger remove-member-btn"
		title="Delete User and remove Access Passes"
...
jQuery(document).ready(function($) {
    console.log('Initializing bulk assign system...');
	    ...
	    // Column sorting for the Users table
    $('#llmsgaa-users-table th').on('click', function() {
        const table = $(this).closest('table');
        const tbody = table.find('tbody');
        const rows = tbody.find('.member-row').toArray();
        ...
        rows.sort((a, b) => {
            ...
            // Column 3 (index 3) is Last Activity - use raw timestamp for proper date sorting
            if (columnIndex === 3) {
                aVal = parseInt($(a).data('last-login-raw')) || 0;
                bVal = parseInt($(b).data('last-login-raw')) || 0;
                return isAsc ? aVal - bVal : bVal - aVal;
            } else {
                aVal = $(a).find('td').eq(columnIndex).text().trim().toLowerCase();
                bVal = $(b).find('td').eq(columnIndex).text().trim().toLowerCase();
...
// ===== BULK ASSIGN MODAL WITH PASS SELECTION BY ABHIJIT =====
window.showBulkAssignModal = function(selectedEmails) {
    //console.log('Loading bulk assign modal for:', selectedEmails);
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            const allLicenses = response.data; // each item = one unassigned license/pass row from backend
            const memberCount = selectedEmails.length;

            // 1. Group licenses by course/pass "type"
            // shape: { courseKey: { meta, licenses: [licenseObj, ...] } }
            const passesByCourse = {};
            allLicenses.forEach(lic => {
                // We'll build a unique key per course+start date (like you did in groupedLicenses)
                const courseKey = lic.course_title + '||' + lic.start_date_formatted + '||' + lic.end_date_formatted;

                if (!passesByCourse[courseKey]) {
                    passesByCourse[courseKey] = {
                        course_title: lic.course_title,
                        start_date_formatted: lic.start_date_formatted,
                        end_date_formatted: lic.end_date_formatted,
                        licenses: []
                    };
                }
                passesByCourse[courseKey].licenses.push(lic);
            });

            // How many total licenses exist across ALL selected courses?
            const totalAvailableCount = allLicenses.length;

            // We *could* still precompute summary (optional nice UX)
            const willBeAssignedIfAllSelected = Math.min(memberCount, totalAvailableCount);
            const willNotBeAssignedIfAllSelected = Math.max(0, memberCount - totalAvailableCount);
            ...
            let modalHTML = `
                ...
                ">
                    ...
                    ">
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
                            <h2 style="margin:0;color:#333;font-size:22px;display:flex;align-items:center;gap:8px;">
                                <span style="font-size:20px;">📋</span>
                                <span>Bulk Access Pass Assignment</span>
                            </h2>
                            <button onclick="closeBulkModal()" style="
                                background:none;
                                border:none;
            ...
            // 3. Status alert (green / yellow / red style block)
            if (totalAvailableCount === 0) {
                ...
                `;
            } else if (willBeAssignedIfAllSelected === memberCount) {
                modalHTML += `
                    <div style="
                        background:#d4edda;
                        color:#155724;
                        ...
                        line-height:1.4;
                    ">
                        ✅ All ${memberCount} selected Users can receive an Access Pass (enough passes exist).
                    </div>
                `;
            } else {
                modalHTML += `
                    ...
                    ">
                        <strong>⚠️ Limited Access Passes Available</strong><br>
                        • ${willBeAssignedIfAllSelected} User(s) can get Access Passes<br>
                        • ${willNotBeAssignedIfAllSelected} User(s) may not get Access Passes<br>
                        <small style="display:block;margin-top:5px;">
                            Passes will be used in the order users are listed.
                        </small>
                    </div>
                `;
            }

            // 4. Pass selection list (this is the NEW PART, mirroring single-user modal)
            modalHTML += `
                <div style="margin-bottom:20px;">
                    <h3 style="font-size:16px;margin:0 0 10px;font-weight:600;">Select Access Pass(es) to assign:</h3>
            `;

            Object.keys(passesByCourse).forEach((key, idx) => {
                const passGroup = passesByCourse[key];
                const availableForThisPass = passGroup.licenses.length;
                const checkboxId = `bulk-pass-${idx}`;
                        ...
                        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px;">
                            <div style="flex:1;">
                                <div style="font-weight:600;color:#000;">
                                    ${passGroup.course_title}
                                </div>
                                <div style="font-size:12px;color:#555;margin-top:4px;">
                                    Starts: ${passGroup.start_date_formatted}<br>
                                    Ends: ${passGroup.end_date_formatted}<br>
                                    <span style="color:#007bff;font-weight:500;">
                                        ✦ ${availableForThisPass} Access Pass${availableForThisPass!==1?'es':''} available
                                    </span>
                                </div>
                            </div>
                            <div>
                                <input
                                    type="checkbox"
                                    class="bulk-pass-checkbox"
                                    id="${checkboxId}"
                                    data-license-ids='${JSON.stringify(passGroup.licenses.map(l => l.ID))}'
            ...
            // 5. Assignment order preview (keep, so admin knows who is being assigned)
            modalHTML += `
                <div style="margin-bottom:20px;">
                    <h3 style="font-size:16px;margin:0 0 10px;font-weight:600;">Assignment Order:</h3>
                    <ol style="
                        background:#f8f9fa;
                        padding:15px 15px 15px 35px;
                        border-radius:4px;
            ...
            // kill old modal then add new
            $('#bulk-assign-modal').remove();
            $('body').append(modalHTML);
            $('#bulk-assign-modal').fadeIn(300);

            // 7. Hook up Assign button
            $('#bulk-assign-confirm-btn').off('click').on('click', function() {

                // Collect all selected pass groups
                let finalLicenseIDs = [];
                $('.bulk-pass-checkbox:checked').each(function() {
                    const idsJson = $(this).attr('data-license-ids');
                    try {
                        const idsArr = JSON.parse(idsJson);
                        if (Array.isArray(idsArr)) {
                            finalLicenseIDs = finalLicenseIDs.concat(idsArr);
                        }
                    } catch(e) {
                        console.error('Invalid data-license-ids JSON', e);
                    }
                });

                //console.log('finalLicenseIDs=', finalLicenseIDs);
                
                if (finalLicenseIDs.length === 0) {
                    alert('Please select at least one Access Pass type to assign.');
                    return;
                }

                // Send to processor
                processBulkAssignment(selectedEmails, finalLicenseIDs);
            });
...
window.processBulkAssignment = function(memberEmails, licenseIds) {
    ...
    $.ajax({
        ...
        data: {            
            ...
            emails: memberEmails,           // array of emails
            license_ids: licenseIds,        // ordered array of pass IDs (by your selection)
        ...
        const html = `
            ...
            <form id="add-member-form">
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <select id="member-role" aria-label="Role" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 6px; font-size: 14px;">
...
function showAssignLicenseModal(email, memberName, licenses) {
    ...
    $('#llmsgaa-csv-export-btn').on('click', function() {
        const rows = [];
        
        // Header row
        rows.push(['Name', 'Email', 'Role', 'Status', 'Last Activity']);
        
        // Data rows from the table
        $('.member-row').each(function() {
            const $row = $(this);
            const name = $row.find('td:eq(1) strong').text().trim();
            const email = $row.data('email');
            const role = $row.find('.role-select').val() || 'member';
            const status = name === 'Pending Invitation' ? 'pending' : 'active';
            const lastActivity = $row.find('td:eq(3) span').text().trim();
            
            rows.push([name, email, role, status, lastActivity]);
        });
        ...
        link.download = 'group-members-' + new Date().toISOString().split('T')[0] + '.csv';
        link.click();
    });
