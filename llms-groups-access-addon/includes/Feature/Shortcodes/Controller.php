...
/*updated for v2.7+ 
Each group order stores two extra meta fields:
source_pass_id → the pass that created the order
source_pass_identifier → the identifier extracted from the pass (e.g., “DC1234” or fallback “Pass-123”)
v2.7a
replace $product_title with $sku when building $order_title
recognize DC|ZR for $pass_identifier
*/
...
public static function handle_redeem_pass() {
    ...
    // Extract pass identifier from title (e.g., DC1234, ZR1234)
    $pass_identifier = '';
    if ( preg_match( '/\b(.+\d+)\b/i', $pass->post_title, $matches ) ) {
    ...
    foreach ( $items as $item ) {
        ...
        for ( $i = 0; $i < $qty; $i++ ) {
            ...
            $order_title = sprintf( '%s for %s', $sku, $pass_identifier );
