 ...
 * wp_ajax_llmsgaa_bulk_assign_sequential funcation updated with my new code (wp_ajax_llmsgaa_bulk_assign_sequential) and commented old funcation
 */
...
// ===========Sequential Bulk Assign Licenses Handler by Abhijit =====================
add_action( 'wp_ajax_llmsgaa_bulk_assign_sequential', function() {
    if ( ! wp_verify_nonce( $_POST['nonce'] ?? '', 'llmsgaa_unified_actions' ) ) {
        wp_send_json_error( 'Invalid nonce' );
    }
    ...
    $emails = isset( $_POST['emails'] ) ? array_map( 'sanitize_email', $_POST['emails'] ) : [];
    $selected_ids = isset($_POST['license_ids']) ? array_values(array_map('intval', (array) $_POST['license_ids'])) : [];
    $group_id = intval( $_POST['group_id'] ?? 0 );
    ...
    if (!empty($selected_ids)) {
        // Use exactly what the UI selected
        $available_licenses = array_map(function ($id) {
            return (object) [
                'ID'           => $id,
                'course_title' => get_post_field('post_title', $id) ?: 'Access Pass'
            ];
        }, $selected_ids);
    } else {
        // original fallback
    ...
    foreach ( $emails as $email ) {
        ...
        // Try the assignment for this specific pass ID
        $result = LLMSGAA\Feature\Shortcodes\UnifiedMemberManager::assign_license( $license->ID, $email );
