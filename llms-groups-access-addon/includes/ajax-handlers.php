...
add_action('wp_ajax_llmsgaa_get_available_licenses_detailed', function() {
    if (!wp_verify_nonce($_POST['nonce'] ?? '', 'llmsgaa_unified_actions')) {
        wp_send_json_error('Invalid nonce');
    }
...
// Multi-Pass Bulk Assignment Handler
add_action('wp_ajax_llmsgaa_bulk_assign_multi_pass', function() {
    global $wpdb;
    ...
    if (!$group_id || !is_array($assignments) || empty($assignments)) {
        wp_send_json_error('Invalid request data');
    }
    ...
    // Group assignments by email to handle multiple passes per member
    $assignments_by_email = [];
    foreach ($assignments as $assignment) {
        $email = sanitize_email($assignment['email']);
        if (!isset($assignments_by_email[$email])) {
            $assignments_by_email[$email] = [];
        }
        $assignments_by_email[$email][] = $assignment;
    }
    
    // Process each member
    foreach ($assignments_by_email as $email => $member_assignments) {
...
$is_member = $wpdb->get_var($wpdb->prepare(
    "SELECT COUNT(*) FROM {$wpdb->prefix}lifterlms_user_postmeta 
     WHERE post_id = %d AND user_id = %d AND meta_key = '_group_role'",
    $group_id,
    $user->ID
));
...
if (!$is_member) {
    ...
    // Also add the enrollment status
    $wpdb->insert(
        "{$wpdb->prefix}lifterlms_user_postmeta",
        [
            'user_id' => $user->ID,
        ...
        // Process each pass assignment for this member
        $courses_assigned = [];
        
        foreach ($member_assignments as $assignment) {
            $pass_id = intval($assignment['pass_id']);
            $course_id = $assignment['course_id'];
            
            // Get current assigned licenses for this pass
            $assigned_licenses = get_post_meta($pass_id, 'llmsgaa_assigned_licenses', true);
            if (!is_array($assigned_licenses)) {
                $assigned_licenses = [];
            }
            
            // Check if already assigned to this pass
            $already_assigned = false;
            foreach ($assigned_licenses as $license) {
                if (isset($license['user_id']) && $license['user_id'] == $user->ID) {
            ...
            if ($already_assigned) {
                continue; // Skip if already has this pass
            }
            ...
            $assigned_licenses[] = $license_data;
            
            // Update pass metadata
            update_post_meta($pass_id, 'llmsgaa_assigned_licenses', $assigned_licenses);
            
            // Track assigned course
            if ($course_id && !in_array($course_id, $courses_assigned)) {
        ...
        // Enroll in all assigned courses
        foreach ($courses_assigned as $course_id) {
            if (!$course_id) continue;
            
            // Check if already enrolled
            $enrollment_status = llms_get_user_postmeta($user->ID, $course_id, '_status');
            
            if ($enrollment_status !== 'enrolled') {
                // Enroll the user
                llms_enroll_student($user->ID, $course_id, 'group_access_pass_multi');
                
                // Set enrollment metadata
                llms_update_user_postmeta($user->ID, $course_id, '_enrollment_trigger', 'group_bulk_assignment');
                llms_update_user_postmeta($user->ID, $course_id, '_group_id', $group_id);
            }
        }
        ...
        $results['assigned'][] = [
            'email' => $email,
            'user_id' => $user->ID,
            'courses_count' => count($courses_assigned)
        ];
    }
    
    // Send summary message
    if ($results['assigned_count'] > 0) {
        $member_count = count($results['assigned']);
        $results['message'] = sprintf(
            'Successfully assigned %d passes to %d member%s',
            $results['assigned_count'],
            $member_count,
            $member_count !== 1 ? 's' : ''
        );
